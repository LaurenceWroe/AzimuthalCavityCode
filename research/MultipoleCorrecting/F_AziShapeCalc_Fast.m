...
for ii = 1:length(theta)
    ...
    y = 0; 
    for jj = 1:length(m)
        y = y+BessVals(jj)*besselj(m(jj),k*xAr)*cos(m(jj)*thetaVal+PhiVals(jj));
    end
    ...
    if ii == 1 % Some special considerations for first block
        ...
        Roots(ii).Gradient = 1/k*y_prime_num./y_prime_dom; % Determine the gradient
        ...
        Roots(ii).Gradient = 1/k*y_prime_num./y_prime_dom;
