...
RF_Folder ='/Users/wroe/cernbox2/Code/RF_Track/Scripts/uniform_beam/Maps2/260310_LBand_TM_410/'; f = 1.3e9;
...
%
RF_Bool = 0;
RF_Folder_2 ='/Users/wroe/cernbox2/Code/RF_Track/Scripts/uniform_beam/Maps2/260310_TM_610_Pill/';
RF_Struct_2 = load(fullfile(RF_Folder_2,RF_File));


Multipoles = 1+RF_Bool; % set number of multipoles you want to look at
...
if RF_Bool == true
  ...
  T_E_2 = RF_FieldMap_CINT(real(RF_Struct_2.E1_Mat)*SF, real(RF_Struct_2.E2_Mat)*SF, real(RF_Struct_2.E3_Mat)*SF, ...
                     1i*imag(RF_Struct_2.B1_Mat)*SF, 1i*imag(RF_Struct_2.B2_Mat)*SF,  1i*imag(RF_Struct_2.B3_Mat)*SF, ...
                 RF_Struct.loc_x*1e-3,  RF_Struct.loc_y*1e-3, ... % this is bottom left corner of mesh, m
                 RF_Struct.d1*1e-3, ... % dx, m
                 RF_Struct.d2*1e-3, ... % dy, m
                 RF_Struct.d3*1e-3, ... % dz, m
    ...
    Ta = linspace(0,2*pi,721);
    for Ti = Ta
       [E,B] = T_E_2.get_field(a*1000*cos(Ti), a*1000*sin(Ti), T_E.get_length()/2*1000, 0); % x,y,z,t (mm, mm/c)
        Ez_circ = [ Ez_circ ; E(3) ];
    end
    FFT = 2*fft(Ez_circ); FFT(1) = FFT(1)/2;
    FFT_f = FFT(1:length(FFT)/2)/(length(FFT));
