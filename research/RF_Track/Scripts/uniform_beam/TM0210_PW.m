...
RF_Folder = '/Users/wroe/cernbox2/Code/RF_Track/Scripts/uniform_beam/Maps/TM0210Acc_PipeAzi/';
RF_File = 'TM0210.dat';
LPipe = 0.06; LM = 0.02;
##LPipe = 0.04; LM = 0.04;
...
##X_offsets = 0*23;
N = 2001;
T_Vary = linspace(-a3(1,1,end),a3(1,1,end),N);
T_Static = zeros(1,N);

Y = zeros(1,N);
Z = linspace(-a3(1,1,end),a3(1,1,end),N);
...
for ii = 1:length(X_offsets)
      ...
      [E_Static,B_Static] = V.get_field(X,Y,Z,T_Static);
      Ez_RF_Static = E_Static(:,3);
      Pz_Static(ii) = trapz(Z/1000,Ez_RF_Static);

      [E_Vary,B_Vary] = V.get_field(X,Y,Z,T_Vary);
      Ez_RF_Vary = E_Vary(:,3);
...
p = polyfit (X_offsets,Pz_Static/1e6, logical ([1, 0, 1]))
plot(X_offsets,p(1)*X_offsets.^2+p(3),'--')
...
p = polyfit (X_offsets,Pz_Vary/1e6, logical ([1, 0, 1]))
plot(X_offsets,p(1)*X_offsets.^2+p(3),'--')
