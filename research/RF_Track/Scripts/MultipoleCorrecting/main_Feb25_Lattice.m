...
RF_Folder = '/Users/wroe/cernbox2/Code/RF_Track/Scripts/MultipoleCorrecting/Maps/260216_Cancel10/';
...
% Compare the autophase result to prediction
Test_Time = 0;
if Test_Time == 1
    NoParticles = 360;
    Test_B = zeros(NoParticles, 6);

    P_arr = logspace(-1,3,6)
    P_arr = [0.1, 1, 2.5, 5, 10, 50,100];
    for jj = 1:length(P_arr)
        phi_0 = linspace(0,RF_Cav.get_period,NoParticles);
        Test_B(:,6) = P_arr(jj);
        Test_B(:,5) = phi_0;
        Phase_Test = Bunch6d (mass, 1, charge, Test_B);
        B_Test_f = L.track(Phase_Test);
        M_i = Phase_Test.get_phase_space("%S %E %x %Px %y %Py %Pc %t %Pz %id");
        M_f = B_Test_f.get_phase_space("%S %E %x %Px %y %Py %Pc %t %Pz %id");
        if length(M_f) ~= length(M_i)
          if ~isempty(M_f)
            [~,sorted] = sort(M_f(:,10))
            M_f = M_f(sorted,:);
            M_f_new = nan(NoParticles,10);
            count = 1;
            for ii = 1:length(M_i)
              if count<=length(M_f)
                if M_f(count,10) == ii-1
                  M_f_new(ii,:) = M_f(count,:);
                 count = count+1;
                endif
                end
            end
          else
            M_f_new = zeros(1,10); M_f = zeros(1,10); phi_0 = 0; M_i = zeros(1,10);
            end
        else
            M_f_new = M_f;
        end

        survived = M_i(sort(M_f(:,10)+1),:);
        figure(7); hold all; plot(phi_0,M_f_new(:,9)-M_i(:,9),'-')
        max(M_f_new(:,9)-M_i(:,9))
    end
end
...
for jj = 1:length(RF_Phase)
    ...
    Ez_on3 = E_field_on(:,3);
    if jj == 1
        Ez_on3_phase = E_field_on(:,3);
        Ez_on3_stat = E_field_on_stat(:,3);
        Vz_on_field = trapz(Za/1e3,Ez_on3_phase/1e6); %[MV]
        Vz_on_field_stat = trapz(Za/1e3,Ez_on3_stat/1e6);
    end
    %figure(); hold all; plot(Za,Ez_on3); plot(Za,Ez_on3_stat)
    ...
    for ii = 1:rings
      ...
      if jj == 1
##        tilde_G_0 = 1; tilde_G_1 = 0.3464; tilde_G_2 = 0.0546;
...
##          tilde_G_0 = 1; tilde_G_1 = XOptim(1); tilde_G_2 = XOptim(2); % do this to best fit!
      end
...
##        MaxPzError = max(max(abs(delta_Pz_ring-DeltaPzForm)./DeltaPzForm))*100;
...
##      plot(r_arr,Volt)
      ...
      for ii = 1:NoR
##        plot(theta_arr*180/pi,Volt_t(:,ii)./Vz_on_field,'-','color',colours(ii,:),'linewidth',1.2,'handlevisibility','off')
##        if ii == 1
##          plot(theta_arr*180/pi,Volt_t(:,ii)./Volt_t(:,ii),'-','color',colours(ii,:),'linewidth',1.2,'displayname',['r = ' num2str(r_arr(ii)) ' mm'])
##          else
##        plot(theta_arr*180/pi,Volt_t(:,ii)/(-1.6953),'-','color',colours(ii,:),'linewidth',1.2,'displayname',['r = ' num2str(r_arr(ii)) ' mm'])
##        endif

            if ii == 1
              plot(theta_arr*180/pi,Volt_t(:,ii)./Vz_on_field,'-','color',colours(ii,:),'linewidth',1.2,'displayname',['r = ' num2str(r_arr(ii)) ' mm'])
             else
              plot(theta_arr*180/pi,Volt_t(:,ii)./Vz_on_field-0.9e-5,'-','color',colours(ii,:),'linewidth',1.2,'displayname',['r = ' num2str(r_arr(ii)) ' mm'])
            endif
