...
%% Define RF Cavity %%
RF_Folder = '/Users/wroe/cernbox2/Code/RF_Track/Scripts/MultipoleCorrecting/Maps/CouplerNoCorrection/';
%RF_Folder = '/Users/wroe/cernbox2/Code/RF_Track/Scripts/MultipoleCorrecting/Maps/NoCoupler/';
RF_File = '150kMB_0_5_Steps.dat';
...
P_f = L.autophase(P_i);
v_z = mean(B0.get_phase_space('%Vz'));

L{2}.get_t0;
L{2}.set_phid(RF_Phase)
...
PW_Bool = 0;
if PW_Bool == 1
     X_pos = reshape(B(:,1),rings,p_per_ring);
     Y_pos = reshape(B(:,3),rings,p_per_ring);
    ...
    Delta_p_x_Lor = charge*trapz(Za/1e3,Ex_t_Mod-By_t_Mod*v_z*c,3)/1e3; % [keV/c]
    Delta_p_y_Lor = charge*trapz(Za/1e3,Ey_t_Mod+Bx_t_Mod*v_z*c,3)/1e3; % [keV/c]
    subplot(3,2,3); hold all; plot(T_arr,Delta_p_x_Lor); xlabel('\theta [rad]'); ylabel(['\Delta P_x Lorentz [keV/c]' ; ' '; ' ']); grid on; box on;
    subplot(3,2,4); hold all; plot(T_arr,Delta_p_y_Lor); xlabel('\theta [rad]'); ylabel(['\Delta P_y Lorentz [keV/c]' ; ' '; ' ']); grid on; box on;
    ...
    for ii = 1:length(Za)
     ...
     subplot(3,2,[1 2]); hold all; plot(T_arr,Delta_p_z); xlabel('\theta [rad]'); ylabel(['\Delta P_z [MeV/c]' ; ' '; ' ']); grid on; box on;
   ...
   grad_x = gradient(V_X,X); grad_y = gradient(V_Y,Y);
    ...
    for ii = 1:rings
     ...
     subplot(3,2,5); hold all; plot(T_arr, Delta_p_x'); xlabel('\theta [rad]'); ylabel('\Delta P_x PW [keV/c]'); grid on; box on;
     subplot(3,2,6); hold all; plot(T_arr, Delta_p_y'); xlabel('\theta [rad]'); ylabel('\Delta P_y PW [keV/c]'); grid on; box on;

##    for ii = 1:length(Za)
####        [E_field_t,B_field_t] = L{2}.get_field(X_pos, Y_pos, Za(ii), Za(ii)); % x,y,z,t (mm, mm/c)
##        X_Grid = linspace(-r_beam, r_beam, 101); % mm
##        Y_Grid = linspace(-r_beam, r_beam, 101); % mm
##        [Z,X] = meshgrid(Za,Xa);
##        L{2}.set_phid(90)
##        [E_field_t,B_field_t] = L{2}.get_field(X_mat(:), Y_mat(:), Za(ii), Za(ii)); % x,y,z,t (mm, mm/c)
##        Ex_t = reshape(E_field_t(:,1),size(X_mat)); Ex_t_Mod(:,:,ii) = reshape(Ex_t,length(R_arr),length(T_arr));
...
##     end
     ...
     ##     figure();
##     plot(R_arr,V_Mat)
   %figure(); plot(squeeze(Ez_t_Mod(:,1,:))')
     %figure(); plot(R_arr,V_Ez_t_Mod(:,1,:))
##     for ii = 1:length(R_arr)
##          V_Test(ii) = trapz(Za/1e3,squeeze(Ez_t_Mod(ii,1,:))');
##     end
##     figure(); plot(R_arr,V_Test)
##     figure(); hold all;
##      for ii = 1:rings
##          plot(T_arr, V_Mat(ii,:)); xlabel('\theta [rad]'); ylabel('V [MV]')
##       end
##
##      figure(); hold all;
##      for ii = 1:length(T_arr)
##          plot(R_arr, V_Mat(:,46)); xlabel('r [mm]'); ylabel('V [MV]')
##       end
end
...
for ii = 1:rings
    ...
    subplot(2,2,[1, 2]); hold all; plot(theta_arr,delta_Pz_ring(ii,:),'-');
    delta_Px_ring(ii,:) = delta_Px((ii-1)*p_per_ring+1:ii*p_per_ring);
    subplot(2,2,3); hold all; plot(theta_arr,1000*delta_Px_ring(ii,:),'-');
    delta_Py_ring(ii,:) = delta_Py((ii-1)*p_per_ring+1:ii*p_per_ring);
    subplot(2,2,4); hold all; plot(theta_arr,1000*delta_Py_ring(ii,:),'-');
endfor
xlim([0 360]); xlabel('\theta [rad]'); ylabel(['\Delta P_y [keV/c]'; ' ' ; ' ']);
subplot(2,2,3); xlim([0 360]); xlabel('\theta [rad]'); ylabel(['\Delta P_x [keV/c]'; ' ' ; ' ']);
subplot(2,2,[1, 2]); xlim([0 360]); xlabel('\theta [rad]'); ylabel(['\Delta P_z [MeV/c]' ; ' '; ' ']);
##subplot(2,2,2); plot(M0(:,4),M0(:,6),'x'); xlabel('Px [MeV/c]'); ylabel('Py [MeV/c]');
